...
export class TemporaryJammerEntityManager {
  private static instance: TemporaryJammerEntityManager | null = null;
  ...
  public static GetInstance(viewer?: Cesium.Viewer): TemporaryJammerEntityManager {
    if (!TemporaryJammerEntityManager.instance) {
      if (!viewer) {
        throw new Error(
          "Viewer must be provided for the first GetInstance call"
        );
      }
      TemporaryJammerEntityManager.instance = new TemporaryJammerEntityManager(viewer);
    }
    return TemporaryJammerEntityManager.instance;
  }
